...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null } => {
  const token = localStorage.getItem('URAXLAW-API-TOKEN');
  const coreVersion = localStorage.getItem('URAXLAW-CORE-VERSION');
  const apiVersion = localStorage.getItem('URAXLAW-API-VERSION');
  const webuiTitle = localStorage.getItem('URAXLAW-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('URAXLAW-WEBUI-DESCRIPTION');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('URAXLAW-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('URAXLAW-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('URAXLAW-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('URAXLAW-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('URAXLAW-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('URAXLAW-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('URAXLAW-WEBUI-DESCRIPTION');
      }
    ...
    logout: () => {
      localStorage.removeItem('URAXLAW-API-TOKEN');
