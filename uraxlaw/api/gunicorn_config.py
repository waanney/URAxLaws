...
import os
import logging
from uraxlaw.kg.shared_storage import finalize_share_data
from uraxlaw.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to uraxlaw.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to uraxlaw.log
...
def on_starting(server):
    ...
    # Log the location of the UraxLaw log file
    print(f"UraxLaw log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("uraxlaw", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up uraxlaw submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("uraxlaw."):
